...
/**
 * Server-side proxy for the backend NL→SQL endpoint.
 *
 * The browser talks only to this route; this route talks to the backend and
 * attaches the `CHAT_API_KEY` from the server environment, so the key never
 * reaches the client bundle. When no key is configured (local development) the
